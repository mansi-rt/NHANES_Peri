...
data pt;
 set permdata.peri_taste_n2155_250102;
run;
*/

/**** 
1/29/2025, LinHY
send for PI (results wihout weightings)

peri_g2 was not associated with all salty and bitter disablity 
*********/;

ods rtf file='T:\LinHY_project\NHANES\oral_health\results\peri_taste_diab_Noweight_250129_send.rtf' bodytitle;
...
title 'peri status (yes/no) vs. salty and bitter disabiltiy';
proc freq data=pt;
  table peri_g2 * (disab_salty_tp disab_salty_wm disab_salty_all disab_bit_tp disab_bit_wm disab_bit_all)/chisq nocol;
run;


title 'peri status (yes/no) vs. salty and bitter disabiltiy';
proc freq data=pt;
  table peri_stage * (disab_salty_tp disab_salty_wm disab_salty_all disab_bit_tp disab_bit_wm disab_bit_all)/nocol chisq fisher;
 run;
ods rtf close;
